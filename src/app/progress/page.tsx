...
import { ArrowLeft, ChevronDown, Flame, CheckCircle2, Circle, Calendar, TrendingUp, BarChart3, Loader2 } from 'lucide-react';
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between bg-slate-950/40 p-2.5 rounded-2xl border border-slate-900 max-w-md mx-auto shadow-inner">
          <button
            ...
            className="px-4 py-2 bg-slate-900/60 hover:bg-slate-800/40 hover:text-white border border-slate-800 rounded-xl text-xs font-bold uppercase tracking-wider transition-colors cursor-pointer"
          >
            ← Prev
          </button>
          <div className="text-center">
            ...
            {selectedDate === new Date().toISOString().split('T')[0] && (
              <span className="text-[10px] text-emerald-400 font-extrabold uppercase tracking-wider block mt-0.5">Today</span>
            )}
          </div>
          <button
            onClick={() => handleDateChange(1)}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="glass-panel border-slate-800/80 rounded-2xl p-6 flex flex-col items-center justify-center shadow-2xl relative overflow-hidden">
            ...
            <div className="relative w-36 h-36">
              ...
              <div className="absolute inset-0 flex items-center justify-center flex-col pointer-events-none">
                ...
                <span className="text-[9px] uppercase tracking-wider font-extrabold text-slate-500 mt-0.5">done</span>
              </div>
            </div>
            <div className="mt-4 text-center">
              <p className="text-base font-bold text-slate-100">{completedCount} of {dailyProgress.habits.length} habits</p>
              <p className="text-xs text-slate-400 font-medium mt-1">completed on this day</p>
            </div>
          </div>
          ...
          <div className="glass-panel border-slate-800/80 rounded-2xl p-6 flex flex-col items-center justify-center shadow-2xl relative overflow-hidden">
            ...
            <p className="text-4xl font-black text-white tracking-tight">{dailyProgress.summary.currentStreak}</p>
            <p className="text-xs uppercase tracking-wider font-extrabold text-slate-500 mt-2">Day Streak</p>
            <p className="text-[10px] text-orange-400/80 font-bold mt-1">Consistency is key!</p>
          </div>
          ...
          <div className="glass-panel border-slate-800/80 rounded-2xl p-6 shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center bg-slate-950/40 p-2.5 rounded-xl border border-slate-900/60">
                ...
                <span className={`text-xs font-bold ${dailyProgress.comparison.change.rate >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  {dailyProgress.comparison.change.rate >= 0 ? '+' : ''}{dailyProgress.comparison.change.rate.toFixed(1)}%
                </span>
              </div>
              <div className="flex justify-between items-center bg-slate-950/40 p-2.5 rounded-xl border border-slate-900/60">
                <span className="text-xs font-semibold text-slate-400">Habits Done</span>
                <span className={`text-xs font-bold ${dailyProgress.comparison.change.completed >= 0 ? 'text-emerald-400' : 'text-red-400'}`}>
                  {dailyProgress.comparison.change.completed >= 0 ? '+' : ''}{dailyProgress.comparison.change.completed}
                </span>
              </div>
              <div className="pt-3 border-t border-slate-800/50">
                <p className="text-[11px] text-slate-500 font-semibold">Yesterday: {dailyProgress.comparison.previousDay.completed} habits ({dailyProgress.comparison.previousDay.completionRate.toFixed(0)}%)</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="glass-panel border-slate-800/80 rounded-2xl p-6 shadow-2xl">
          <h3 className="text-base sm:text-lg font-bold text-white mb-4 tracking-tight">Today&apos;s Habits Detail</h3>
            ...
            {dailyProgress.habits.map((habit) => (
              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-xl flex items-center justify-center border" style={{ backgroundColor: habit.color + '15', borderColor: habit.color + '30' }}>
                    {habit.icon ? (
        ...
        {dailyProgress.upcomingReminders.length > 0 && (
          <div className="glass-panel border-slate-800/80 rounded-2xl p-6 shadow-2xl">
            <h3 className="text-base sm:text-lg font-bold text-white mb-4 tracking-tight">Upcoming Reminders</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {dailyProgress.upcomingReminders.map((reminder) => (
                <div key={reminder.habitId} className="flex items-center justify-between p-3.5 bg-slate-950/40 border border-slate-900 rounded-xl">
                  <span className="text-xs font-bold text-slate-300">{reminder.title}</span>
                  <span className="text-xs text-sky-400 font-bold bg-sky-500/10 px-2.5 py-1 rounded-lg border border-sky-500/20">{reminder.reminderTime}</span>
                </div>
              ))}
            </div>
          </div>
              ...
              <Tooltip
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    return (
                      <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-3.5 min-w-[150px] font-sans">
                        <p className="text-xs font-bold text-slate-100 mb-2">{new Date(payload[0].payload.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            <div className="flex items-center gap-4">
              ...
              <h1 className="text-lg sm:text-xl font-bold text-white tracking-tight">Progress Logs</h1>
            </div>

            {/* Segmented control tabs */}
            <div className="flex bg-slate-950 border border-slate-900 rounded-xl p-1 gap-1 shadow-inner">
              {views.map((v) => (
                <button
                  key={v.value}
